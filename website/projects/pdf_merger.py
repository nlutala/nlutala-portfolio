...
class PDFMerger:
    ...
    def add_pdf_files(self, pdfs: list) -> None:
        self.pdfs = pdfs
        if len(self.pdfs) == 0:
            raise ValueError("There are no PDF files to merge.")
        elif len(self.pdfs) == 1:
            raise ValueError(
                "There is only one PDF file but there needs to be 2 or more PDF files to merge."
            )
    ...
    def merge_pdfs(self, filename: str):
        ...
        for pdf in self.pdfs:
            if pdf.endswith(".pdf"):
                ...
                raise TypeError(
                    "Expected PDF file(s) but a file with another file extension was given instead."
                )
